...
### 1.12 小结  
## 第2章 列表和元组  
本章引入新的概念：数据结构。数据结构是通过某种方式组织在一起的数据元素的集合，数据元素可以是数字或者字符，甚至可以使其他数据结构。  
...
### 7.4 总结
## 第8章 异常  
在编写程序的时候，程序员通常需要辨别事件的正常过程和异常的情况。
### 8.1 什么是异常  
Python用异常对象来表示异常情况。遇到错误后，会引发异常，如果异常对象未被处理货捕捉，程序就会用所谓的回溯（Traceback）终止执行。  
### 8.2 按自己的方式出错  
异常可以在某些东西出错时自动引发。  
#### 8.2.1 raise语句  
...
SyntaxError：在代码未错误形式时引发；  
TypeError：在内建操作或者函数应用于错误类型的对象时引发；  
ValueError：在内建操作或者函数应用于正确类型的对象，但是该对象使用不合适的值时引发；  
ZeroDivisionError：在除法或者模除操作的第二个参数为0时引发  
#### 8.2.2 自定义异常类  
内建的异常类已经包括了大部分的情况，但是有些时候还是需要创建自己的异常类。  
如何创建自己的异常类呢？确保从Exception类继承即可
### 8.3 捕捉异常  
关于异常最有意思的是可以处理它们，使用try/except语句实现对异常的捕捉处理。
### 8.4 不止一个except子句  
一个try语句后面可以跟2+个except语句
### 8.5 用一个块捕捉两个异常  
如果需要捕捉多个类型的异常，那么可以将它们作为元组列出。  
e.g except （ZeroDivisionError,  TypeError, NameError）
### 8.6 捕捉对象  
如果想让程序继续运行，但是又因为某种原因想记录下错误。这个功能就很有用了。  
e.g except （ZeroDivisionError,  TypeError, NameError）as e  

### 8.7 真正的全捕捉  
如果想用一段代码捕捉所有的异常，那么可以在except子句中忽略所有的异常类；  
e.g except:  
### 8.8 万事大吉  
给try/except加一个else子句
### 8.9 最后...  
Finally，用来在可能的异常后进行清理。与try子句联合使用
### 8.10 异常和函数
### 8.11 异常之禅
### 8.12 小结
## 第9章 魔法方法、属性和迭代器  
在名称前后加上两个下划线，__future__。由类似名字组成的集合所包含的方法称之为魔法方法。
### 9.1 准备工作  
### 9.2 构造方法  
首先要讨论的第一个魔法方法是构造方法，构造方法是一个很奇特的名字，代表着类似于以前例子中用过的init的初始化方法。  
构造方法和其他普通方法不同的地方在于，当一个对象被创建后，会立即调用构造方法。
#### 9.2.1 重写一般方法和特殊的构造方法  
通过前面继承的介绍，我们了解到每个类都可能拥有一个或者多个超类，从超类那里继承行为方式。  
如果一个方法在B类的一个实例中被调用，但是在B类中没有找到该方法，那么会从B的超类A里面找。  
在子类中增加功能的最基本的方式就是增加方法，同时也可以重写一些超类的方法来自定义继承的行为。  
重写是继承机制中的一个重要内容，对于构造方法尤为重要。  
构造方法用来初始化新创建对象的状态，大多数子类不仅要拥有自己的初始化代码，还要拥有超类的初始换代码。  
如果一个类的构造方法被重写，那么就需要调用超类的构造方法，否则对象可能不会被正确地初始化。  
#### 9.2.2 调用未绑定的超类构造方法  
使用super函数的方法简单明了。  
在调用一个实例的方法时，该方法的self参数会被自动绑定到实例上。如果直接调用类的方法，那么就没有实例会被绑定，这样就可以自由地提供需要的self参数，这样的方法称为未绑定方法。  
#### 9.2.3 使用super函数  
当前的类和对象可以作为super函数的参数使用，调用函数返回的对象的任何方法都是调用超类的方法，而不是当前类的方法。
### 9.3 成员访问  
__init__是目前为止提到的最重要的特殊方法。  
...
__setitem__(self, key, value):按一定的方式存储和key相关的value，后续可通过__getitem__来获取；  
__delitem__(self, key):对一部分对象使用del语句时被调用，同时必须删除和元素相关的键；  
对于一个序列来讲，如果键是负整数，那么要从末尾开始计数；  
如果键是不合适的类型，会引发一个TypeError异常；  
如果序列的索引时正确的类型，但超出了范围，会引发IndexError异常。  
#### 9.3.2 子类化列表、字典和字符串  
...
### 9.9 小结
## 第10章 充电时刻  
Python的标准安装包括一组模块，称之为标准库。
### 10.1 模块  
#### 10.1.1 模块是程序  
任何Python程序都可以作为模块导入。  
除了从默认的目录中寻找之外，还需要从导入的目录中寻找模块，这样就能导入自己的模块了。  
#### 10.1.2 模块用于定义  
模块在第一次导入到程序中时被执行。  
1.在模块中定义函数  
e.g  
...
a.b()  
2.在模块中增加测试代码  
在模块中添加一些检查模块本身是否正常工作的测试代码还是很有用的  
#### 10.1.3 让你的模块可用  
在理想状况下，sys.path本身就应该包含正确的目录。  
1.将模块放置在正确位置  
将你的文件放到Python解释器查找的目录即可。  
e.g import sys, pprint  
pprint.pprint(sys.path)  
2.告诉编译器去哪里查找  
标准的实现方法PYTHONPATH环境变量中包含模块所在的目录。  
#### 10.1.4 包  
为了组织好模块，你可以将它们分组为包（package）。包基本上就是另外一个模块，有趣的就是它们能包含其他模块。  
### 10.2 探究模块  
#### 10.2.1 模块中有什么  
探究模块最直接的方式就是在Python解释器中研究它们。当然，前提是先导入模块。  
1.dir  
查看模块包含的内容可以使用dir函数，它会将对象的所有特性列出（函数、类、变量）;  
2.__all__变量  
#### 10.2.2 用help获取帮助  
#### 10.2.3 文档  
#### 10.2.4 使用源代码  
...
argv：命令行参数，包括脚本名称；  
exit([arg])：退出当前的程序，可选参数未给定的返回值或者错误信息；  
...
#### 10.3.2 os
os模块为你提供了访问多个操作系统服务的功能。os和它的子模块os.path还包括一些用于检查、构造、删除目录和文件的函数，以及一些处理路径的函数。
#### 10.3.3 fileinput  
fileinput模块让你轻松遍历文本文件的所有行。  
fileinput.input是其中最重要的函数，它会返回能够用于for循环遍历的对象。
#### 10.3.4 集合、堆、和双端队列  
1.集合  
set，由序列构建的，主要用于成员资格检查。  
集合中元素的顺序是随意的；  
可使用标准的集合操作，交集和并集；  
e.g a.union(b)  
a|b  
...
#### 10.3.7 shelve  
shelve模块可以满足在文件中存储数据的大部分需求。  
shelve模块中最有趣的函数时open。  
#### 10.3.8 re  
re(regular expression)模块包含对正则表达式的支持。  
1.什么是正则表达式  
...
b：二进制模式（可与其他模式共同使用）  
+：读/写模式（可与其他模式共同使用）  
‘+’可以用到任何模式中，指明读和写都是允许的。  
‘b’模式改变处理文件的方法，在声音剪辑或者图像文件处理过程中需要用到‘b’  
#### 11.1.2 缓冲  
open函数的第三个参数控制着文件的缓冲。  
0/False: I/O就是无缓冲的（所有的读写操作都直接针对硬盘）；  
1/True : I/O就是有缓冲的（Python使用内存来代替硬盘，让程序更快，在使用flush或者close的时候才会更新硬盘上的数据）
### 11.2 基本文件方法  
#### 11.2 1 读和写  
文件（或流）最重要的能力是提供或者接受数据。如果有一个名为f的类文件对象，那么久可以用f.write方法和f.read方法写入和读取数据。  
f.write(string)会将string追加到文件中已存在部分的后面；  
读取很简单，只要记得告诉流要读多少字符（字节）即可；  
PS：不指定要读取的字节数时，默认读取所有内容。  
#### 11.2.2 管式输出  
在UNIX的shell中，使用管道（|）可以在一个命令后面续写其他的多个命令。  
e.g cat filename.txt|python jiaoben.py|sort  
cat filename.txt:把filename.txt的内容写到标准输出  
python jiaoben.py:运行Python的jiaoben.py脚本，从标准输入读入，把结果写入到标准输出  
sort:从标准输入读取所有的文本，按字母排序，然后把结果写入标准输出  
管道命令（|）将一个命令的标准输出和下一个命令的标准输入连在一起  
#### 11.2.3 读写行  
file.readline读取单独的一行（从当前位置开始直到一个换行符出现，也读取这个换行符）；  
不加任何参数：默认读取一行；  
非负整数n：读取最大n个字节；  
readlines方法可以读取一个文件中的所有行并将其作为列表返回。  
writelines方法和readlines方法相反：传给它一个字符串的列表，它会把所有的字符串写入文件。  
PS：程序不会增加新行，需要自己添加。  
没有writeline方法，只能用write。  
#### 11.2.4 关闭文件  
应该牢记使用close方法关闭文件。写入过的文件总是应该关闭，是因为Python可能会缓存写入的数据。  
如果想要确保文件被关闭了，那么应该使用try/finally语句；  
try:  
	文件操作  
finally:  
file.close()  
在Python2.5之后的Python版本中中可以使用如下语句：  
with open('filename.txt') as a:
do_something(a)  
with语句在打开文件并将其赋值到变量上。之后就可以将数据写入语句体重的文件。  
### 11.3 对文件内容进行迭代  
对文件内容进行迭代以及重复执行一些操作是最常见的文件操作之一。  
#### 11.3.1 按字节处理  
最常见的对文件内容进行迭代的方法是while循环中使用的read方法。  
#### 11.3.2 按行操作  
处理行使用的方法和处理字符一样，使用readline方法  
#### 11.3.3读取所有的内容  
可以使用read方法一次读取整个文件，或者使用readlines方法  
#### 11.3.4 使用fileinput实现懒惰迭代  
在需要对一个非常大的文件进行迭代行的操作时，readlines会占用太多的内存。  
这个时候可以用while循环和readline方法来替代。  
e.g for line in fileinput.input(filename):  
process(line)  
#### 11.3.5 文件迭代器  
文件对象是可迭代的。  
e.g f = open(filename)  
